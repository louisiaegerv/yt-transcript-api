...
def main():
    ...
    if not transcript:
        print("Transcript is empty or unavailable", file=sys.stderr)
        sys.exit(1)
    ...
    print(f"Entry type: {type(transcript[0])}", file=sys.stderr)
    ...
    for i, entry in enumerate(transcript[:5]):
        ...
        if isinstance(entry, dict):
            ...
            print(f"  Attributes: text, start, duration", file=sys.stderr)
            print(f"  text: {repr(entry.text)}", file=sys.stderr)
            print(f"  start: {entry.start}", file=sys.stderr)
            print(f"  duration: {entry.duration}", file=sys.stderr)
    ...
    for i, entry in enumerate(transcript):
        if isinstance(entry, dict):
            text = entry.get('text', '')
            has_start = 'start' in entry
            has_text = 'text' in entry
        else:
            text = entry.text
            has_start = hasattr(entry, 'start')
            has_text = hasattr(entry, 'text')
        
        if not has_start or not has_text:
            print(f"WARNING: Entry {i} missing required attributes", file=sys.stderr)
        if '\n' in text:
